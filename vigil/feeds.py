...
def scrape_abuseipdb(url: str) -> list[dict]:
    ...
    if not api_key:
        print("[vigil] ABUSEIPDB_API_KEY not set — skipping abuseipdb feed")
        return []
...
def scrape_otx_alienvault(url: str) -> list[dict]:
    ...
    if not api_key:
        print("[vigil] OTX_API_KEY not set — skipping OTX AlienVault feed")
        return []
...
def scrape_pulsedive(url: str) -> list[dict]:
    ...
    if not api_key:
        print("[vigil] PULSEDIVE_API_KEY not set — skipping Pulsedive feed")
        return []
...
def scrape_greynoise(url: str) -> list[dict]:
    ...
    if not api_key:
        print("[vigil] GREYNOISE_API_KEY not set — skipping GreyNoise feed")
        return []

    # GreyNoise bulk noise context endpoint requires paid API
    print("[vigil] GreyNoise bulk feed requires paid API — returning empty")
...
def bundle_iocs(iocs: list[dict], feed_name: str, chunk_size: int = 50) -> list[dict]:
    """Split IOC dicts into vigil-format bundle dicts."""
    bundles: list[dict] = []
...
def ingest_to_server(
    ...
    """Upload IOC bundles to an vigil server. Returns count of successful uploads."""
