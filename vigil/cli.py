"""CLI: vigil — privacy-preserving threat intelligence tool."""
...
_CONFIG_PATH = Path.home() / ".vigil" / "config.json"


def _load_config() -> dict:
    """Load saved config from ~/.vigil/config.json."""
...
def _get_api_url(explicit: str | None) -> str | None:
    ...
    if explicit:
        return explicit
    env = os.environ.get("VIGIL_API_URL")
    if env:
        return env
...
def _get_api_key(explicit: str | None) -> str | None:
    ...
    if explicit:
        return explicit
    env = os.environ.get("VIGIL_API_KEY")
    if env:
        return env
...
@click.group()
def main():
    """vigil — collective defense for industries.

    \b
    Full loop:
      vigil up                          # start server + scrape live feeds
      vigil report incident_iocs.json   # give data, get intelligence
    """
    pass
...
@main.command()
def init():
    """Set up vigil — save your server URL and API key so you never type them again."""
    config = _load_config()
    ...
    click.echo(f"\n  You're ready. Try: vigil report <file>")
    click.echo()
...
@click.option("--api-url", default=None, help="Server URL (default: from vigil init)")
@click.option("--api-key", default=None, help="API key (default: from vigil init)")
...
def upload(file, api_url, api_key, industry, org_size, role, epsilon, yes, json_output):
    ...
    if not api_url:
        click.echo("  No server URL configured. Run: vigil init")
        raise SystemExit(1)
    ctx = ContribContext(
...
def scrape(feed, list_feeds, dry_run, api_url, api_key):
    ...
    if not dry_run and not api_url:
        click.echo("  No server URL configured. Run: vigil init  (or use --dry-run)")
        raise SystemExit(1)
    ...
    feed_status_path = Path.home() / ".vigil" / "feed_status.json"
    feed_status: dict = {}
...
@click.option("--db", default="sqlite+aiosqlite:///vigil.db", help="Database URL")
...
def up(port, host, db, skip_feeds, vertical):
    """Start vigil: server + live threat feeds. One command, full loop.
    ...
    try:
        ...
        click.echo("  Server requires: pip install vigil[server]")
        raise SystemExit(1)

    api_url = f"http://127.0.0.1:{port}"

    # Save config so `vigil report` works without flags
    config = _load_config()
    ...
    if vertical:
        ...
        click.echo(f"  ┌─────────────────────────────────────────┐")
        click.echo(f"  │  vigil up — {v.display_name[:27]:27s}│")
        click.echo(f"  └─────────────────────────────────────────┘")
    ...
    if not skip_feeds:
        ...
        click.echo()
        click.echo(f"    vigil report <your_incident_file.json>")
        click.echo()
        click.echo("  ──────────────────────────────────────────")
        click.echo()
        ...
        click.echo(f"  Config saved — run: vigil report <file>")
        click.echo()
...
def serve(port, host, db):
    """Start the vigil server (without feed ingestion)."""
    try:
        import uvicorn
        from .server.app import create_app
    except ImportError:
    ...
    app = create_app(db_url=db)
    app.state.port = port
    click.echo(f"  vigil server starting on {host}:{port}")
...
def psi_query(peer, file):
    ...
    try:
        ...
        click.echo("  PSI requires: pip install vigil[crypto]")
    except Exception as e:
...
def status(api_url, api_key):
    ...
    if api_url:
        ...
        click.echo("  Server:  not configured (run: vigil init)")
    ...
    if feed_status_path.exists():
        ...
        click.echo("\n  Feed status: no scrape history (run: vigil scrape --dry-run)")
    click.echo()
...
def sources():
    ...
    click.echo(f"\n  vigil data sources ({len(ALL_SOURCES)} total: {implemented} implemented, {planned} planned)")
    click.echo("  " + "=" * 70)
...
@click.option("--output", "-o", default=None, help="Output file (default: vigil_export.json)")
@click.option("--api-url", default=None, help="Server URL (default: from vigil init)")
@click.option("--api-key", default=None, help="API key (default: from vigil init)")
def export(fmt, output, api_url, api_key):
    ...
    output = output or f"vigil_export.{fmt}"
...
def purge(older_than):
    ...
    click.echo()
    click.echo("  Or connect to the vigil DB and run a migration.")
    click.echo()
